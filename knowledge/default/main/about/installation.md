...
- This directory will contain all your PRISM Oracle files, like the legacy root folder structure:
  ...
  - `.env` - Your API keys
  - `settings.json` - Your PRISM Oracle settings
...
> mixed in the vector database. Note that this will DELETE ALL of PRISM Oracle's memory.
...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the PRISM Oracle repository. 
...
- PRISM Oracle is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make PRISM Oracle work with its full development environment.
...
2. You don't necessarily need API keys: PRISM Oracle can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running PRISM Oracle without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download PRISM Oracle
- You can clone the PRISM Oracle repository (https://github.com/agent0ai/agent-zero) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the PRISM Oracle releases [here](https://github.com/agent0ai/agent-zero/releases).
...
2. In the terminal, navigate to your PRISM Oracle folder using **"cd"** command. Replace the path with your actual PRISM Oracle folder path.
~~~
...
3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"a0"** - short for PRISM Oracle. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
~~~
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox PRISM Oracle.
- PRISM Oracle only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows PRISM Oracle to 
...
- PRISM Oracle needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/a0` volume to the location of your local PRISM Oracle folder.
- Run the Docker container following the instructions.

## 5. Run the local PRISM Oracle instance
Run the PRISM Oracle with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the PRISM Oracle interface.

## 6. Configure PRISM Oracle
Now we can configure PRISM Oracle - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure PRISM Oracle.

## 7. Configure PRISM Oracle RFC
PRISM Oracle needs to be configured further to redirect some functions to the Docker container. This is crucial for development as A0 needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on PRISM Oracle.
