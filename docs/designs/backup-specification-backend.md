# PRISM Oracle Backup/Restore Backend Specification

## Overview
This specification defines the backend implementation for PRISM Oracle's backup and restore functionality, providing users with the ability to backup and restore their PRISM Oracle configurations, data, and custom files using glob pattern-based selection. The backup functionality is implemented as a dedicated "backup" tab in the settings interface for easy access and organization.
...
- Integrates with PRISM Oracle's established error handling and toast notification system
...
backup_section: SettingsSection = {
    ...
    "description": "Backup and restore PRISM Oracle data and configurations using glob pattern-based file selection.",
    "fields": [
        {
            "id": "backup_create",
            "title": "Create Backup",
...
> **⚠️ CRITICAL FILE NOTICE**: The `{agent_root}/.env` file contains essential configuration including API keys, model settings, and runtime parameters. This file is **REQUIRED** for PRISM Oracle to function properly and should always be included in backups alongside `settings.json`. Without this file, restored PRISM Oracle instances will not have access to configured language models or external services.
...
The BackupService leverages PRISM Oracle's existing file operation helpers which already support RFC (Remote Function Call) routing for development mode. This ensures seamless operation whether running in direct mode or with container isolation.
...
#### PRISM Oracle Internal Dependencies
The backup system requires these PRISM Oracle helper modules:
...
#### Integration with PRISM Oracle Error System
The backup system integrates with PRISM Oracle's existing error handling infrastructure:

```python
from python.helpers.errors import format_error
from python.helpers.print_style import PrintStyle

# Follow PRISM Oracle's error handling patterns
try:
...
PRISM Oracle's existing task scheduler could be extended to support automated backups:
...
The backup system uses the same version detection method as PRISM Oracle's main UI:
...
- **Environment Details**: User, timezone, working directory, runtime mode, PRISM Oracle root path
...
- **Compatibility Data**: PRISM Oracle version and environment for restoration validation
...
This enhanced backend specification provides a production-ready, comprehensive backup and restore system with advanced metadata tracking, real-time progress monitoring, and intelligent file management capabilities, all while maintaining PRISM Oracle's architectural patterns and security standards.
...
- **Testing Verified**: BackupService initializes correctly and detects PRISM Oracle root paths
...
**Result:** Critical files like `.env` are now properly backed up when explicitly specified, ensuring PRISM Oracle configurations are preserved.
...
The PRISM Oracle backup system is now production-ready and battle-tested! 🚀
...
- Works seamlessly between different PRISM Oracle installations
...
The PRISM Oracle backup system now provides:
