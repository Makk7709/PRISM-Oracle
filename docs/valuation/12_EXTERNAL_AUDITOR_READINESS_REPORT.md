...
**HEAD distant transmis (audit factuel)** : `2f3eb0e6` (synchronise `origin` au 15 mai 2026).
**HEAD precedents** : `1d05531a` (alignement HEAD), `c990cc55` (verrouillage securite).
**Base main** : `fab5689a` (snapshot d'analyse 5 mai 2026).
**Source** : `docs/valuation/11_FACTUAL_INTEGRITY_AUDIT.md` (audit hostile total).
**Mise a jour du 15 mai 2026** : micro-verrouillage post-push, traitement complet de RES-3 (annexe AE-11 livree), traitement partiel de RES-1 (`docs/SPEC_MULTI_USER_WORKSPACE.md` sanitise). Detail dans `13_RESERVES_RESOLUTION_AND_DISCLOSURE.md`.
...
|---|---|---|---|---|
| RES-1 | Prenoms clients reels (`coralie`, `dominique`, `laurianne`) dans 4 fichiers trackes hors perimetre mission d'audit initial : `deploy/docker-compose.yml`, `docs/GUIDE_DEPLOIEMENT_ENTREPRISE.md`, `docs/SPEC_MULTI_USER_WORKSPACE.md`, `tests/test_organization_canonical.py` | **Modere** (PII faible, mais visible par auditeur prudent) | **Statut au 15 mai 2026** : (1) `docs/SPEC_MULTI_USER_WORKSPACE.md` **sanitise** dans le commit de micro-verrouillage (prenoms remplaces par `user1`/`user2`/`admin` dans l'exemple JSON structurel) ; (2) `deploy/docker-compose.yml` **conserve en l'etat** (code Docker operationnel, modification casserait les paths Samba runtime) ; (3) `docs/GUIDE_DEPLOIEMENT_ENTREPRISE.md` **conserve en l'etat** (47 occurrences referencant docker-compose.yml, sanitization isolee creerait des incoherences) ; (4) `tests/test_organization_canonical.py` **conserve en l'etat** (test applicatif d'isolation cross-tenant, modification necessite re-execution complete TDD). Detail et justification : `13_RESERVES_RESOLUTION_AND_DISCLOSURE.md`. | Decision apporteur : accepter la transmission en l'etat (mention strategique des deploiements pilotes deja produits, cf. RES-2) OU mission ulterieure dediee de re-anonymisation profonde du couple `compose + GUIDE + tests`. |
| RES-2 | Mentions clients **DICA FRANCE**, **Tarmac**, **Centrale Lille / Pr Zoubeir Lafhaj** dans documents valorisation (`RAPPORT_TECHNIQUE_*.md`, `DOSSIER_COMMISSAIRE_*.md`) | **Faible juridiquement** (les mentions sont strategiques pour defendre la borne haute) ; **modere reglementairement** si NDA / clauses confidentialite client non verifiees | **Statut au 15 mai 2026** : verification NDA externe au perimetre technique. La presente mission ne peut pas modifier les mentions clients sans risque de degrader la defense de valorisation. Decision strategique formalisee dans `13_RESERVES_RESOLUTION_AND_DISCLOSURE.md` §2 (acceptation explicite OU anonymisation a posteriori). | Apporteur : confirmer NDA AVANT transmission externe. Si NDA non couvrant, anonymiser ulterieurement en "Client A (PME conseil)", "Client B (incubateur public)", "Client C (chaire universitaire)". |
| RES-3 | Audit licence interne date 2025-02-08 (>14 mois) sans rapport `pip-licenses` annexe | **Modere** | **Statut au 15 mai 2026** : **TRAITE**. Re-scan execute via `venv/bin/pip-licenses 5.5.1` le 2026-05-15. Annexe AE-11 livree (`docs/annexes-externes/AE-11_pip-licenses_2026-05-15.md`, `.json`, `README.md`). `legal/THIRD_PARTY_NOTICES.txt` reformule v2 avec mention explicite des 5 LGPL / 5 MPL 2.0 / cas `pyphen` triple-licence / cas `espeakng-loader` (GPL natif isolable hors perimetre valorise). | Reserve **levee** pour le scan du 2026-05-15. Validite 30 jours. Re-scan recommande si transmission posterieure au 2026-06-14. |
