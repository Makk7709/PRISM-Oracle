# PRISM Oracle Connectivity Guide

This guide covers the different ways to connect to PRISM Oracle from external applications, including using the External API, connecting as an MCP client, and enabling agent-to-agent communication.

**Note:** You can find your specific URLs and API tokens in your PRISM Oracle instance under `Settings > External Services`.
...
PRISM Oracle provides external API endpoints for integration with other applications. These endpoints use API key authentication and support text messages and file attachments.

### `POST /api_message`

Send messages to PRISM Oracle and receive responses. Supports text messages, file attachments, and conversation continuity.
...
PRISM Oracle includes an MCP Server that allows other MCP-compatible clients to connect to it. The server runs on the same URL and port as the Web UI.
...
Below is an example of a `mcp.json` configuration file that a client could use to connect to the PRISM Oracle MCP server. 
...
PRISM Oracle's A2A Server enables communication with other agents using the FastA2A protocol. Other agents can connect to your instance using the connection URL.

### A2A Connection URL

To connect another agent to your PRISM Oracle instance, use the following URL format. 
