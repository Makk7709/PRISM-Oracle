...
class LiteLLMChatWrapper(SimpleChatModel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Set Korev model config as instance attribute after parent init
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        apply_rate_limiter_sync(self.korev_model_conf, str(msgs))
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        # Apply rate limiting if configured
        await apply_rate_limiter(self.korev_model_conf, str(msgs))
    ...
    async def unified_call(
        ...
        **kwargs: Any,
        ...
        limiter = await apply_rate_limiter(
            self.korev_model_conf, str(msgs_conv), rate_limiter_callback
        )

        # Prepare call kwargs and retry config (strip Korev-only params before calling LiteLLM)
        call_kwargs: dict[str, Any] = {**self.kwargs, **kwargs}
        max_retries: int = int(call_kwargs.pop("korev_retry_attempts", 2))
        retry_delay_s: float = float(call_kwargs.pop("korev_retry_delay_seconds", 1.5))
...
class BrowserCompatibleChatWrapper(ChatOpenRouter):
    ...
    async def _acall(
        ...
        **kwargs: Any,
        ...
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self._wrapper.korev_model_conf, str(messages))
...
class LiteLLMEmbeddingWrapper(Embeddings):
    model_name: str
    kwargs: dict = {}
    korev_model_conf: Optional[ModelConfig] = None
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self.korev_model_conf, " ".join(texts))
    ...
    def embed_query(self, text: str) -> List[float]:
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self.korev_model_conf, text)
...
def _adjust_call_args(provider_name: str, model_name: str, kwargs: dict):
    ...
    if provider_name == "openrouter":
        kwargs["extra_headers"] = {
            "HTTP-Referer": "https://korev.ai",
            "X-Title": "KOREV Evidence",
        }
