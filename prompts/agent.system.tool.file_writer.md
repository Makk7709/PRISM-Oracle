...
- Create documents in specific styles (stratégie, juridique, scientifique, brevet...)
...
## 🎨 TEMPLATES PDF DISPONIBLES — KOREV Evidence

**IMPORTANT**: Choisis le template selon la demande utilisateur !

| Template | Utilisation | Mots-clés utilisateur |
|----------|-------------|----------------------|
| `consulting_premium` | Rapport stratégique premium | "stratégie", "consulting", "MECE", "deck", "due diligence" |
| `legal_formal` | Document juridique formel | "juridique", "tribunal", "greffe", "avocat", "contrat" |
| `scientific_academic` | Publication académique | "scientifique", "recherche", "étude", "paper" |
| `patent_ip` | Brevet INPI/EPO | "brevet", "patent", "INPI", "invention" |
| `financial_audit` | Rapport financier/audit | "financier", "audit", "bilan", "DCF" |
| `executive_brief` | Note de synthèse | "executive", "synthèse", "direction", "brief" |
| `medical_clinical` | Rapport médical | "médical", "clinique", "diagnostic", "patient" |
| `technical_doc` | Documentation technique | "technique", "API", "architecture", "doc" |

### Sections suggérées par template

**Consulting Premium**: Executive Summary → Situation Analysis → Key Findings (MECE) → Strategic Options → Recommendation → Implementation → Risks

**Legal Formal**: INTITULÉ → PARTIES → EXPOSÉ DES FAITS → DISCUSSION EN DROIT → PAR CES MOTIFS → DISPOSITIF

**Scientific Academic**: Abstract → Introduction → Methods → Results → Discussion → Conclusion → References

**Patent IP**: TITRE → DOMAINE TECHNIQUE → ART ANTÉRIEUR → PROBLÈME → SOLUTION → AVANTAGES → REVENDICATIONS

---

## Exemples

### Rapport Stratégique Premium — EXEMPLE COMPLET
